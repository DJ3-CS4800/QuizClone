...
public class ApiController {
    ...
    public ResponseEntity<?> getAllDecks(HttpSession session) {
        ...
        if(userID == null) return ResponseEntity.ok(Map.of("error", "User not logged in"));
    ...
    public ResponseEntity<?> getDeck(@RequestBody DeckSelection deckSelection, HttpSession session) {
        ...
        // Check if user has any progress on the deck, if not, create a new progress entry for the user in the deck_progress table
        if(currUserID != null){
            isOwner = studyDeck.getOwnerID() == ((Long) session.getAttribute("userID"));
            DeckProgress deckProgress = 
                deckProgressRepo.findByUserIDAndDeckID(currUserID, deckSelection.getDeckID());
    ...
    public ResponseEntity<?> createDeck(@RequestBody StudyDeckCreateRequest studyDeckCreateRequest, HttpSession session) {
        Long userID = (Long) session.getAttribute("userID");
    ...
    public ResponseEntity<?> updateDeck(@RequestBody StudyDeckEditRequest studyDeckEditRequest, HttpSession session) {
        ...
        if(studyDeck == null) return ResponseEntity.ok(
            Map.of("error", "Deck does not exist"));

        if(studyDeck.getOwnerID() != userID) return ResponseEntity.ok(
